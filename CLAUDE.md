...
| `GET /lava/specs` | 300s | Chain RPC | All chain specs with `{ index, name, icon }`. Used by frontend `useChainNames` hook; `icon` is an absolute URL (`INFO_ICONS_BASE_URL` + `/chains/<slug>.svg`) for cross-origin consumers |
| `GET /provider-rewards?from=&to=&groupBy=` | 300s | Indexer MV | Per-provider (default) or per-(provider, spec) QoS-adjusted reward shares. Required: `from`, `to` (YYYY-MM-DD, exclusive end, max 6 months). Optional: `specs` (chain filter), `groupBy` (`provider` default, or `spec`). In `provider` mode the GraphQL pools all chains before computing `cbrt(...)` — required for parity with delta's adjustedRewards because the formula is non-linear. In `spec` mode each row is a (provider, spec) pair with within-spec `rewardShare`. Returns `{ meta, data: [{ provider, [spec]?, moniker, relays, cus, qosCus, avgLatency, avgAvailability, avgSync, avgLatencyExc, avgAvailabilityExc, avgSyncExc, adjustedRewards, rewardShare }] }`. Replaces lava-ops Job 1. For USD estimates use `/provider-estimated-rewards` |
| `GET /provider-estimated-rewards?block=&spec=` | 1800s (86400s with `block`) | Chain RPC + CoinGecko | Per-provider actual chain rewards from `estimated_provider_rewards` RPC, grouped by spec. Optional `block=N` for historical queries (requires archive node; threads `x-cosmos-block-height` header). Optional `spec=X` filters response. USD values always use CURRENT CoinGecko price, even for historical blocks (matches lava-ops Job 2 behavior) — `meta.priceLavaUsd` / `meta.priceTimestamp` surface this explicitly. Returns `{ meta: { block, spec, priceLavaUsd, priceTimestamp }, data: [{ provider, moniker, rewards: [{ chain, spec, tokens, total_usd }], total_usd }] }`. Sorted by total_usd descending. Replaces lava-ops Job 2 |
| `GET /provider-estimated-rewards/blocks?count=` | 86400s | Chain RPC | Historical monthly-17th blocks for the block-selector UI. Maps each month's 17th at 15:00 UTC to a block height via `fetchBlockAtTimestamp`. Default 12 months, max 24. Returns `{ data: [{ height, time, date }] }` |
...
| `RPC_BATCH_SIZE` | `25` | API — max concurrent chain-RPC calls per batch loop (raise on dedicated endpoints; lower on public RPC to avoid rate limiting) |
