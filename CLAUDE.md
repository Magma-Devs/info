...
| `GET /validators-and-rewards` | 7200s | Chain RPC + CoinGecko | All bonded validators with distribution rewards, outstanding rewards, estimated rewards, delegations, and unbonding — all converted to USD. 5 RPC calls per validator, batched 3 at a time (15 concurrent). Returns `{ data: { height, datetime, validators: [...] } }`. Replaces jsinfo's `/lava_mainnet_validators_and_rewards` |
