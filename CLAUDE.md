...
| `GET /provider-rewards?from=&to=` | 300s | Indexer MV | Per-provider QoS-adjusted reward shares. Required: `from`, `to` (YYYY-MM-DD, exclusive end, max 6 months). Optional: `specs` (chain filter). Returns `{ meta: { from, to, totalAdjustedRewards }, data: [{ provider, moniker, relays, cus, qosCus, avgLatency, avgAvailability, avgSync, avgLatencyExc, avgAvailabilityExc, avgSyncExc, adjustedRewards, rewardShare }] }`. Formula and QoS averaging match delta's adjustedRewards exactly. For USD estimates use `/provider-estimated-rewards` |
| `GET /provider-estimated-rewards` | 1800s | Chain RPC + CoinGecko | Per-provider actual chain rewards from `estimated_provider_rewards` RPC, grouped by spec. Calls chain RPC per provider (batched 5 at a time). Returns `{ data: [{ provider, moniker, rewards: [{ chain, spec, tokens, total_usd }], total_usd }] }`. Sorted by total_usd descending. Replaces lava-ops Job 2 |
