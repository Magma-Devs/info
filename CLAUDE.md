...
| `GET /provider-rewards?from=&to=` | 300s | Indexer MV + Chain RPC + CoinGecko | Per-provider QoS-adjusted reward distribution. Required: `from`, `to` (YYYY-MM-DD, max 6 months). Optional: `specs` (chain filter). Returns `{ meta: { from, to, lavaUsdPrice, providerPoolLava, totalAdjustedRewards }, data: [{ provider, moniker, relays, cus, adjustedRewards, rewardShare, estimatedRewardsLava, estimatedRewardsUsd }] }`. Estimated LAVA/USD amounts derived from on-chain provider reward pool balance. Uses historical LAVA price when date range is in the past. Price gracefully degrades to null on CoinGecko failure |
