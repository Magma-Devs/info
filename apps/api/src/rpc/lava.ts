...
// ── Claimable rewards (jsinfo-shape) ─────────────────────────────────────────
...
// ── Validators & rewards (jsinfo-shape) ──────────────────────────────────────
...
const EMPTY_BREAKDOWN: TokenBreakdown = { tokens: [], total_usd: 0 };

async function rewardsToBreakdown(rewards: RewardCoin[] | undefined): Promise<TokenBreakdown> {
 ...
 *
 * Each validator requires 5 chain RPC calls; processes 3 validators at a time
 * (15 concurrent requests) to respect rate limits. With ~50 validators this
 * completes in ~20-30s cold, well within the 6h cache window.
 */
